  ...
  Book.findById(req.params.bookid, (err, result)=> {
    User.findById(result.owner, (err, user)=> {
      ...
      res.render('books/detail', {
        ...
        isOwners: req.user && result.owner._id == req.user.id
      });
    });      
  });
};
...
exports.removeBook = (req, res, next)=> {
  Book.findOne({_id: req.params.bookid}, (err, book)=> {
    if (err) return next(err);
      ...
      book.remove();
      User.findByIdAndUpdate(book.owner._id, {$pull: {books: {_id: book._id}}}, (err, data)=>{
        if (err) return next(err);
        res.redirect('/account/books');
      });
    } 
  ...
  Book.findOne({_id: req.params.bookid}, (err, book)=> {
    if (book.owner._id != req.user.id) {
      ...
      res.redirect('/account');
        ...
        if (err) return next(err);
        req.flash('success', { msg: 'Updated trade status.' });
        res.redirect('/book/' + req.params.bookid);
      });
    }  
  });
